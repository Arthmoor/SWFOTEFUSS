SWFoteFUSS - Fixed Up Smaug Source
==================================

SWFoteFUSS is an ongoing project to find and eliminate as many bugs in the SWFotE (Star Wars: Fate of the Empire) MUD codebase as possible, while also modernizing and updating the code where needed.
...
With SWFOTEFUSS, you no longer need to worry about that. The code used the dlsym functionality to handle the required lookups. It isn't even necessary to have DECLARE_DO_FUN statements anywhere. Just insert, compile, and create the command online. It's that easy now.
...
Configuring the Makefile should no longer be a necessity for SWFOTEFUSS as all of the options that could be have been rewritten for automated detection of code features.
...
Unpack the archive in whatever folder you will be working in. It will default to creating a "swfotefuss" directory. Change the name of this as you see fit.

On a Linux or CYGWIN based system:

Change to the swfotefuss/src directory and type: make

On FreeBSD or OpenBSD:

Change to the swfotefuss/src directory and type: gmake
